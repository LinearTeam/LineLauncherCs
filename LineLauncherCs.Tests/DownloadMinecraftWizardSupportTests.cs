...
public class DownloadMinecraftWizardSupportTests
{
    ...
    [Fact]
    public void BuildLoaderSelectionState_AllowsVanillaWhileCatalogIsLoading()
    {
        var state = DownloadMinecraftWizardSupport.BuildLoaderSelectionState(
            new DownloadMinecraftWizardContext(@"C:\Games\.minecraft", "1.18.2", GameVersionDisplayType.Release),
            null,
            null,
            null,
            true,
        ...
        Assert.Null(state.Selection.FabricVersion);
        Assert.Null(state.Selection.ForgeVersion);
        Assert.Null(state.Selection.OptiFineVersion);
