...
public class TaskSystemTests
{
    ...
    [Fact]
    public async Task TaskManager_DoesNotRunFinalizationAfterSiblingFails()
    {
        using var manager = new TaskManager(2);
        ...
        var parent = manager.CreateParent("parent");
        parent.CreateSubTask<object>("processor", 0, async (_, _, _) =>
        {
            started.SetResult();
            await allowFailure.Task;
            throw new InvalidOperationException("processor failed");
        });
        var finalization = parent.CreateSubTask<object>("finalization", 0, (_, _, _) =>
        {
        ...
        await WaitForConditionAsync(() => parent.State == TaskState.Faulted && finalization.IsFinished);

        Assert.False(finalizationStarted);
        Assert.Equal(TaskState.Canceled, finalization.State);
    }
