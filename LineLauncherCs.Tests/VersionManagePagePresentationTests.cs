...
public class VersionManagePagePresentationTests
{
    ...
    [Fact]
    public void ShouldRefreshOnActivation_RespectsPendingFlagAndThirtySecondThreshold()
    {
        var now = DateTime.UtcNow;

        Assert.True(VersionManagePagePresentation.ShouldRefreshOnActivation(true, true, now, now));
        Assert.True(VersionManagePagePresentation.ShouldRefreshOnActivation(true, false, now.AddSeconds(-31), now));
        Assert.False(VersionManagePagePresentation.ShouldRefreshOnActivation(false, true, now.AddSeconds(-31), now));
