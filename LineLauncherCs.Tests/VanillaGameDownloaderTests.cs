...
public class VanillaGameDownloaderTests : IDisposable
{
    ...
    [Fact]
    public async Task GetVersionManifestAsync_RefreshesCachedUnknownVersionsAfterRecovery()
    {
        using var scope = new TestFileSystemScope();
        var rootPath = scope.CreateDirectory("gameRoot");
        var versionDirectory = scope.CreateDirectory(Path.Combine("gameRoot", "versions", "custom"));
        File.WriteAllText(Path.Combine(versionDirectory, "custom.jar"), string.Empty);
        File.WriteAllText(Path.Combine(versionDirectory, "custom.json"), """
            {
              "id": "custom",
              "mainClass": "main",
              "libraries": [],
        ...
        HttpUtils.Transport = new DelegateHttpRequestTransport((_, _) => Task.FromResult(
            manifestAvailable
                ? CreateManifestResponse("1.21.9")
        ...
        var initial = Assert.Single(await versionManager.ScanVersionsAsync(rootPath));
        Assert.Equal(LocalGameVersionEntry.UnknownClientVersionId, initial.ClientVersionId);

        var refreshed = new TaskCompletionSource<IReadOnlyList<LocalGameVersionEntry>>(
            TaskCreationOptions.RunContinuationsAsynchronously);
        void OnUnknownVersionsRescanned(string refreshedRoot, IReadOnlyList<LocalGameVersionEntry> versions)
        {
            if (string.Equals(refreshedRoot, rootPath, StringComparison.OrdinalIgnoreCase))
            {
                refreshed.TrySetResult(versions);
            }
        }

        VersionManager.UnknownVersionsRescanned += OnUnknownVersionsRescanned;
        try
        {
            manifestAvailable = true;
            await new VanillaGameDownloader().GetVersionManifestAsync();

            var updated = Assert.Single(await refreshed.Task.WaitAsync(TimeSpan.FromSeconds(10)));
            Assert.Equal("1.21.9", updated.ClientVersionId);
            Assert.Equal("1.21.9", Assert.Single(await versionManager.GetCachedOrScanVersionsAsync(rootPath)).ClientVersionId);
        }
        finally
        {
            VersionManager.UnknownVersionsRescanned -= OnUnknownVersionsRescanned;
