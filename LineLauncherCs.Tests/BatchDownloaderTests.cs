...
public class BatchDownloaderTests : IDisposable
{
    ...
    [Fact]
    public void EnsureLibrariesDownloaded_ThrowsWhenABatchContainsFailures()
    {
        var exception = Assert.Throws<InvalidOperationException>(() =>
            VanillaGameSubTaskFactory.EnsureLibrariesDownloaded(
                new BatchDownloadResult(SuccessCount: 8, FailedCount: 1, SkippedCount: 34)));

        Assert.Contains("1 required library", exception.Message, StringComparison.Ordinal);
    }

    [Fact]
    public async Task DownloadAsync_RerequestsFileOnceWhenHashValidationFails()
    {
        using var scope = new TestFileSystemScope();
        var savePath = scope.GetPath("library.zip");
        ...
        var result = await BatchDownloader.DownloadAsync(
            new BatchDownloadOptions<string>
            {
                Files = ["library"],
                GetSavePath = _ => savePath,
                GetDownloadUrl = _ => "https://example.com/library.zip",
                GetHash = _ => "expected-sha1"
            },
            CancellationToken.None,
            progress: null,
            runtime: new BatchDownloadRuntime
            {
                DownloadFileAsync = async (_, path, _, _) =>
                {
                    ...
                    await File.WriteAllTextAsync(path, "invalid content");
                },
                ComputeSha1 = (_, _) => "actual-sha1"
            });

        Assert.Equal(2, downloadAttempts);
        Assert.Equal(new BatchDownloadResult(0, 1, 0), result);
        Assert.False(File.Exists(savePath));
    }

    [Fact]
    public async Task DownloadAsync_UsesFallbackSourceWhenPreferredSourceFailsHashValidation()
    {
        using var scope = new TestFileSystemScope();
        var savePath = scope.GetPath("library.zip");
        ...
        var result = await BatchDownloader.DownloadAsync(
            new BatchDownloadOptions<string>
            {
                Files = ["library"],
                GetSavePath = _ => savePath,
                GetDownloadUrl = _ => "https://mirror.example/library.zip",
                GetDownloadUrlCandidates = _ =>
                    ["https://mirror.example/library.zip", "https://official.example/library.zip"],
                GetHash = _ => "official-sha1"
            },
            CancellationToken.None,
            progress: null,
            runtime: new BatchDownloadRuntime
            {
                ...
                ComputeSha1 = (path, _) => File.ReadAllText(path).Contains("official", StringComparison.Ordinal)
                    ? "official-sha1"
                    : "mirror-sha1"
            });

        Assert.Equal(["https://mirror.example/library.zip", "https://official.example/library.zip"], requestedUrls);
        Assert.Equal(new BatchDownloadResult(1, 0, 0), result);
    ...
    public async Task DownloadAsync_ReportsFinalProgressOf100()
    {
        ...
        var result = await BatchDownloader.DownloadAsync(
            ...
            CancellationToken.None,
            new CallbackProgress<int>(value => progressValues.Add(value)),
