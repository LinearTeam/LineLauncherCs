...
public class AccountFlowTests
{
    ...
    [Fact]
    public async Task RefreshMicrosoftAvatarsAsync_PrioritizesAccountsWithoutCachedAvatars()
    {
        var accounts = new[]
        {
            new MicrosoftAccount { Name = "Cached" },
            new MicrosoftAccount { Name = "Missing" },
            new MicrosoftAccount { Name = "AlsoCached" }
        };
        var refreshedAccounts = new List<string>();

        await AccountPageSupport.RefreshMicrosoftAvatarsAsync(
            accounts,
            (account, _) =>
            {
                refreshedAccounts.Add(account.Name);
                return Task.FromResult(false);
            },
            account => account.Name != "Missing");

        Assert.Equal(["Missing", "Cached", "AlsoCached"], refreshedAccounts);
    }
