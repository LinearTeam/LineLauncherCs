...
{
    [Fact]
    public void BuildManifestState_DoesNotShowReleaseAsLatestSnapshot()
    {
        var releaseTime = DateTimeOffset.Parse("2026-09-15T00:00:00Z");
        var manifest = new VersionManifestInfo
        {
            Latest = new LatestVersion { Release = "26.3", Snapshot = "26.3" },
            Versions =
            [
                new VersionEntry
                {
                ...
                new VersionEntry
                {
                    ...
                    Time = releaseTime.AddDays(-1)
                }
            ]
        };

        var state = DownloadMinecraftPagePresentation.BuildManifestState(
            manifest, displayType => displayType.ToString());

        Assert.Equal("26.3", state.LatestRelease?.Id);
        Assert.Equal("26.3-rc-3", state.LatestSnapshot?.Id);
    }
