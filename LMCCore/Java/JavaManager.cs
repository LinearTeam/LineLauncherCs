...
是否是JDK：{java.IsJdk}");
    ...
    // Variables
    async private static Task FindViaEnvironmentVariables(HashSet<string> paths) {
        // JAVA_HOME
    ...
    async private static Task FindViaRegistry(HashSet<string> paths) {
        if (!IsWindows())
        {
            return;
        }
    ...
    async private static Task FindViaStandardPaths(HashSet<string> paths) {
        if (IsWindows())
        {
    ...
    async private static Task CheckCommonPaths(HashSet<string> paths, IEnumerable<string> directories) {
    ...
    async private static Task FindViaMinecraft(HashSet<string> paths) {
