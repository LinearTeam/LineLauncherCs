...
public sealed class ProcessLaunchArgumentsStepHandler : IGameLaunchStepHandler
{
    ...
    public Task ExecuteAsync(GameLaunchContext context, CancellationToken cancellationToken)
    {
        ...
        AddConfiguredArguments(context, cb);

        context.CommandBuilder = cb;
        return Task.CompletedTask;
    }

    private static void AddConfiguredArguments(GameLaunchContext context, CommandBuilder commandBuilder)
    {
        var settings = context.LaunchSettings;
        if (settings == null)
        {
            return;
        }

        var memoryMb = settings.AutoAllocateMemory
            ? SystemMemoryInfoProvider.GetCurrent().GetRecommendedAllocationMb()
            : settings.MaxMemoryMb;
        commandBuilder.AddJvmArgument($"-Xmx{memoryMb}M");
        commandBuilder.AddJvmArguments(SplitArguments(settings.JvmArguments));
        commandBuilder.AddGameArguments(SplitArguments(settings.GameArguments));
    }

    private static List<string> SplitArguments(string? arguments)
    {
        return CommandLineArgumentParser.Parse(arguments).ToList();
    }
