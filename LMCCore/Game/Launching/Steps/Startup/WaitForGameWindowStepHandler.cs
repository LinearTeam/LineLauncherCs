...
public sealed class WaitForGameWindowStepHandler : IGameLaunchStepHandler
{
    ...
    private static void TryTerminateProcess(Process process)
    {
        try
        {
            ...
            s_logger.Info($"Launch cancellation requested. Terminating game process {process.Id}.");
            process.Kill(entireProcessTree: true);
        }
        catch (InvalidOperationException)
        {
            // The process may exit after HasExited is checked.
        }
        catch (Exception ex)
        {
            s_logger.Warn($"Failed to terminate cancelled game process: {ex.Message}");
        }
    }
}
