...
public class VanillaGameDownloader(DownloadSourceManager? sourceManager = null)
{
    ...
    private static int s_manifestGeneration;

    internal static event Action? VersionManifestLoaded;

    internal static int ManifestGeneration => Volatile.Read(ref s_manifestGeneration);
    ...
    private async Task<VersionManifestInfo> FetchVersionManifestAsync()
    {
        ...
        var manifest = JsonUtils.Parse(json).Get<VersionManifestInfo>() ??
                       throw new InvalidOperationException("Failed to parse version manifest");
        Interlocked.Increment(ref s_manifestGeneration);
        try
        {
            VersionManifestLoaded?.Invoke();
        }
        catch (Exception ex)
        {
            s_logger.Warn($"Notifying version manifest listeners failed: {ex}");
        }
            ...
            libInfo is { Natives.Count: > 0, Downloads.Classifiers.Count: > 0 })
        {
            ...
            if (libInfo.Natives.TryGetValue(os, out var key) &&
                libInfo.Downloads.Classifiers.TryGetValue(
                    key.Replace("${arch}", PlatformDetector.GetNativeArchitectureToken()),
                    out var fileInfo))
            {
    ...
    public IReadOnlyList<string> GetLibraryDownloadUrlCandidates(string originalUrl)
    {
        return _sourceManager.GetUrlCandidates(originalUrl);
    }
