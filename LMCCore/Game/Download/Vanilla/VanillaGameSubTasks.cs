        ...
        return async (cancellationToken, _, progress) =>
        {
            ...
            var result = await BatchDownloader.DownloadAsync(
                new BatchDownloadOptions<DownloadableFileInfo>
                {
                    ...
                    GetDownloadUrlCandidates = lib => downloader.GetLibraryDownloadUrlCandidates(lib.Url!),
            ...
            EnsureLibrariesDownloaded(result);
            return downloadedLibraries;
        };
    }

    internal static void EnsureLibrariesDownloaded(BatchDownloadResult result)
    {
        if (result.FailedCount == 0)
        {
            return;
        ...
        throw new InvalidOperationException(
            $"Failed to download {result.FailedCount} required library file(s).");
    }
