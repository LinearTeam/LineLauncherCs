...
public class BatchDownloadOptions<TFile>
{
    ...
    /// <summary>
    /// Provides alternate source URLs, ordered from the preferred source to fallbacks.
    /// The first URL is used initially; a different source is tried after a hash mismatch.
    /// </summary>
    public Func<TFile, IReadOnlyList<string>>? GetDownloadUrlCandidates { get; init; }
...
public static class BatchDownloader
{
    ...
    internal static async Task DownloadFileAsync(string url, string savePath, CancellationToken cancellationToken, int maxRetries = 3)
    {
        ...
        for (var retry = 0; retry <= maxRetries; retry++)
        {
            try
            {
                ...
                s_logger.Debug(
                    $"Download response: requested URL={url}; effective URL={response.RequestMessage?.RequestUri ?? new Uri(url)}; " +
                    $"status={(int)response.StatusCode} ({response.ReasonPhrase}); content type={response.Content.Headers.ContentType}; " +
                    $"advertised length={response.Content.Headers.ContentLength?.ToString() ?? "unknown"} bytes.");
                ...
                s_logger.Debug($"Download saved: requested URL={url}; {DescribeDownloadedFile(tempPath)}.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        LogHashValidationFailure(file, tempPath, actualHash, "initial download");
        s_logger.Warn($"{file.DisplayName} failed hash validation after download, retrying once.");
        TryDeleteFile(tempPath);
        var retryUrl = GetHashValidationRetryUrl(file, options);
        var isFallbackSource = !string.Equals(retryUrl, file.DownloadUrl, StringComparison.OrdinalIgnoreCase);
        s_logger.Info(isFallbackSource
            ? $"Retrying {file.DisplayName} from fallback source after hash validation failure: {retryUrl}"
            : $"Re-requesting {file.DisplayName} after hash validation failure.");
        await runtime.DownloadFileAsync(retryUrl, tempPath, cancellationToken, options.MaxRetries);
        ...
        LogHashValidationFailure(file, tempPath, actualHash, "validation retry", retryUrl);
    ...
    private static string GetHashValidationRetryUrl<TFile>(
        PreparedBatchFile<TFile> file,
        BatchDownloadOptions<TFile> options)
    {
        var candidates = options.GetDownloadUrlCandidates?.Invoke(file.Source);
        return candidates?
            .FirstOrDefault(candidate => !string.Equals(candidate, file.DownloadUrl, StringComparison.OrdinalIgnoreCase))
            ?? file.DownloadUrl;
    }

    private static void LogHashValidationFailure<TFile>(
        PreparedBatchFile<TFile> file,
        string tempPath,
        string actualHash,
        string attempt,
    ...
    {
        var expectedHashes = new[] { file.Hash }
            .Concat(file.Hashes ?? [])
            .Where(hash => !string.IsNullOrWhiteSpace(hash))
            .Distinct(StringComparer.OrdinalIgnoreCase);
        // Hash validation failures are actionable even when the launcher uses its normal
        // information-level log filter, so preserve this compact diagnostic in the log.
        s_logger.Warn(
            $"Hash validation diagnostics ({attempt}): file={file.DisplayName}; source URL={sourceUrl ?? file.DownloadUrl}; " +
            $"expected SHA-1=[{string.Join(", ", expectedHashes)}]; actual SHA-1={actualHash}; " +
            $"{DescribeDownloadedFile(tempPath)}.");
    }

    private static string DescribeDownloadedFile(string path)
    {
        try
        {
            ...
            return $"saved length={fileInfo.Length} bytes; first {read} byte(s)={Convert.ToHexString(signature, 0, read)}";
        }
        catch (Exception ex)
        {
            return $"downloaded file metadata unavailable ({ex.GetType().Name}: {ex.Message})";
        }
    }
