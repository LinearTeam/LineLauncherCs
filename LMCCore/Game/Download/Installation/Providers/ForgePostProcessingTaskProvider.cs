...
public sealed class ForgePostProcessingTaskProvider : ModLoaderInstallationTaskProvider
{
    ...
    public override void AddTasks(DownloadInstallationContext context)
    {
        ...
        var forgeLibrariesTask = context.ParentTask.CreateSubTask(
            $"Download Forge libraries ({loader.VersionId})",
            70,
            async (cancellationToken, _, progress) =>
            {
                ...
                {
                    progress.Report(100);
                    return new List<DownloadableFileInfo>();
                }

                return await DownloadAdditionalLibrariesAsync(context, forgeState, cancellationToken, progress);
            },
            dependencies: [installerTask],
            translationKey: "Pages.TaskPage.Tasks.GameInstall.Forge.DownloadLibraries");
        context.Tasks.SetForgeLibrariesTask(forgeLibrariesTask);
        ...
        var processorsTask = context.ParentTask.CreateSubTask(
            ...
            dependencies: [installerTask, versionInfoTask, clientTask, librariesTask, forgeLibrariesTask],
            translationKey: "Pages.TaskPage.Tasks.GameInstall.Forge.RunProcessors");

        context.Tasks.SetForgeProcessorsTask(processorsTask);
    }

    async private Task<List<DownloadableFileInfo>> DownloadAdditionalLibrariesAsync(
        ...
        IProgress<int> progress)
    {
        ...
        await executor(cancellationToken, new Dictionary<Tasks.Model.SubTaskBase, object>(), progress);
