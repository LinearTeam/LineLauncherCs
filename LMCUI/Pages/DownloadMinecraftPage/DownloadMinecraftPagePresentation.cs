...
internal static class DownloadMinecraftPagePresentation
{
        ...
        Func<GameVersionDisplayType, string> displayTypeTextResolver)
    {
        ...
        return new DownloadMinecraftManifestState(
            viewModels,
            viewModels.FirstOrDefault(item =>
                item.DisplayType == GameVersionDisplayType.Release &&
                string.Equals(item.Id, manifest.Latest.Release, StringComparison.OrdinalIgnoreCase))
                ?? viewModels.FirstOrDefault(item => item.DisplayType == GameVersionDisplayType.Release),
            viewModels.FirstOrDefault(item =>
                item.DisplayType == GameVersionDisplayType.Snapshot &&
                string.Equals(item.Id, manifest.Latest.Snapshot, StringComparison.OrdinalIgnoreCase))
                ?? viewModels.FirstOrDefault(item => item.DisplayType == GameVersionDisplayType.Snapshot));
    }
