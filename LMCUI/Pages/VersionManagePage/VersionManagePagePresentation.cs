...
internal static class VersionManagePagePresentation
{
        ...
        Func<VersionDisplayType, LocalGameVersionEntry, (VersionIconKind IconKind, string? IconPath)> resolveVersionIcon)
    {
        ...
        return new VersionRenderData
        {
            ...
            Description = $"{getDisplayTypeText(displayType)} {VersionSummaryPresentation.Build(version, unknownClientVersionText, includeMinecraftLabel: false)}",
            IconKind = iconKind,
            IconPath = iconPath
        };
    }
    ...
    public static bool ShouldRefreshOnActivation(
        bool isPageLoaded,
        bool pendingExternalRefresh,
        DateTime lastRefreshUtc,
        DateTime currentUtc)
    {
        return isPageLoaded &&
               (pendingExternalRefresh || currentUtc - lastRefreshUtc > TimeSpan.FromSeconds(30));
    }
