...
public partial class VersionManagePage : PageBase
{
    ...
    private IReadOnlyList<VersionRenderData> _visibleVersions = [];
    ...
    private readonly static Dictionary<string, IImage> s_builtInVersionIcons = [];
    ...
    {
        _versionItemTemplate = new FuncDataTemplate<VersionRenderData>(
            (renderData, _) => renderData == null ? null : CreateVersionExpander(renderData));
        InitializeComponent();
    ...
    private void OnLoaded(object? sender, RoutedEventArgs e)
    {
        ...
        VersionCatalogRefreshCoordinator.InstalledVersionRefreshed += OnInstalledVersionRefreshed;
        _ = RefreshPageSafelyAsync(useCachedOrInProgress: true);
    }
    ...
    async private Task RefreshPageSafelyAsync(
        bool forceInvalidNotification = false,
        bool useCachedOrInProgress = false)
    {
        try
        {
            await RefreshPageAsync(forceInvalidNotification, useCachedOrInProgress);
        }
        ...
        {
            _logger.Error(ex, "Refreshing version management page");
            if (!_isPageLoaded)
            {
                return;
            }

            var content = I18nManager.Instance.GetString(
                "Pages.VersionManagePage.Errors.ScanFailedContent",
                ex.Message);
    ...
    async private Task RefreshPageAsync(
        bool forceInvalidNotification = false,
        bool useCachedOrInProgress = false)
    {
        _refreshCts?.Cancel();
        ...
        try
        {
            var scannedVersions = useCachedOrInProgress
                ? await _versionManager.GetCachedOrScanVersionsAsync(selectedRoot.RootPath, cancellationToken)
                : await _versionManager.ScanVersionsAsync(selectedRoot.RootPath, cancellationToken);
            versions = scannedVersions.ToList();
        }
        catch (OperationCanceledException)
        {
            return;
    ...
    {
        CurrentRootButton.IsEnabled = selectedRoot != null;
        if (selectedRoot == null)
        {
            CurrentRootHeaderText.Text = I18nManager.Instance.GetString("Pages.VersionManagePage.EmptyState.NoRootHeader");
            CurrentRootDescriptionText.Text = I18nManager.Instance.GetString(
                "Pages.VersionManagePage.EmptyState.NoRootDescription");
            return;
        }

        CurrentRootHeaderText.Text = I18nManager.Instance.GetString("Pages.VersionManagePage.CurrentRoot.Header");
        CurrentRootDescriptionText.Text = versionCount == null
    ...
    private void TopBar_OnSizeChanged(object? sender, SizeChangedEventArgs e)
    {
        var stacked = e.NewSize.Width < 600;
        if (sender is Grid grid)
        {
            grid.RowSpacing = stacked ? 4 : 0;
            grid.ColumnSpacing = stacked ? 0 : 8;
        }

        Grid.SetColumnSpan(CurrentRootButton, stacked ? 2 : 1);
        Grid.SetColumn(ActionCommandBar, stacked ? 0 : 1);
        Grid.SetColumnSpan(ActionCommandBar, stacked ? 2 : 1);
        Grid.SetRow(ActionCommandBar, stacked ? 1 : 0);
    }
    ...
    private void InstallVersionButton_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        MainWindow.NavigatePage(new PageNavigateWay(
            typeof(DownloadMinecraftPageContent),
            selectedItem), NavigateType.New);
    }

    private void OnInstalledVersionRefreshed(VersionCatalogSnapshot snapshot)
    {
        Dispatcher.UIThread.Post(() =>
        {
            var selectedRootPath = _versionManager.GetSelectedRoot()?.RootPath;
            if (!_isPageLoaded || selectedRootPath == null || snapshot.RootPath == null ||
                !string.Equals(
                    VersionManagePagePresentation.NormalizeRootPath(selectedRootPath),
                    VersionManagePagePresentation.NormalizeRootPath(snapshot.RootPath),
    ...
    async private void CurrentRootButton_OnClick(object? sender, RoutedEventArgs e)
    {
        var selectedRoot = _versionManager.GetSelectedRoot();
        ...
        CancellationToken cancellationToken)
    {
        return await Task.Run(() =>
        {
            ...
            foreach (var version in versions)
            {
                ...
                var renderData = BuildVersionRenderData(version);
                PreloadCustomVersionIcon(renderData);
    ...
    private void PreloadCustomVersionIcon(VersionRenderData renderData)
    {
        if (renderData.IconKind != VersionIconKind.File || string.IsNullOrWhiteSpace(renderData.IconPath))
        {
            return;
        }
        ...
        try
        {
            renderData.Icon = LoadVersionIconFile(renderData.IconPath);
        }
        catch (Exception ex) when (IsVersionIconLoadException(ex))
        {
            _logger.Debug($"Unable to load version icon '{renderData.IconPath}': {ex.Message}");
        }
    }
    ...
    private FAIconSource CreateVersionIconSource(VersionRenderData renderData)
    {
        try
        {
            ...
            {
                VersionIconKind.Asset when renderData.IconPath != null => new FAImageIconSource
                {
                    Source = GetBuiltInVersionIcon(renderData.IconPath)
                },
                VersionIconKind.File when renderData.Icon != null => new FAImageIconSource
                {
                    Source = renderData.Icon
                },
    ...
    private static IImage GetBuiltInVersionIcon(string iconPath)
    {
        if (s_builtInVersionIcons.TryGetValue(iconPath, out var icon))
        {
            return icon;
        }

        using var stream = AssetLoader.Open(new Uri($"avares://LMCUI{iconPath}"));
        // Built-in version icons have wide transparent margins around their artwork.
        icon = new CroppedBitmap(new Bitmap(stream), new PixelRect(140, 140, 220, 220));
        s_builtInVersionIcons[iconPath] = icon;
        return icon;
    }

    private static IImage LoadVersionIconFile(string iconPath)
    {
        using var stream = new FileStream(iconPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
        return new Bitmap(stream);
    }

    private static bool IsVersionIconLoadException(Exception exception)
    {
        return exception is FileNotFoundException or DirectoryNotFoundException or IOException or ArgumentException;
    }
    ...
    {
        if (!IsRelevantVersionChange(e.FullPath))
        {
            return;
        }
    ...
    private bool IsRelevantVersionChange(string path)
    {
        if (string.IsNullOrWhiteSpace(_watchedRootPath))
        {
            return false;
        }

        var versionsPath = Path.Combine(_watchedRootPath, "versions");
        var relativePath = Path.GetRelativePath(versionsPath, path);
        ...
        return parts.Length == 2 &&
               (string.Equals(Path.GetExtension(parts[1]), ".json", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(Path.GetExtension(parts[1]), ".jar", StringComparison.OrdinalIgnoreCase));
    }
