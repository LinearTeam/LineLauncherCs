...
internal static class AccountPageSupport
{
    ...
    public async static Task<MicrosoftAvatarRefreshSummary> RefreshMicrosoftAvatarsAsync(
        ...
        Func<MicrosoftAccount, bool>? hasCachedAvatar = null,
        CancellationToken cancellationToken = default,
        TimeSpan? requestInterval = null)
    {
        var hasAvatarUpdate = false;
        var shouldSaveAccounts = false;
        var accountList = hasCachedAvatar == null
            ? accounts.ToList()
            : accounts.OrderBy(hasCachedAvatar).ToList();
        ...
        for (var index = 0; index < accountList.Count; index++)
        {
            ...
            if (requestInterval is { } interval && interval > TimeSpan.Zero && index < accountList.Count - 1)
            {
