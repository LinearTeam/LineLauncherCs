...
internal static class AccountAvatarService
{
    ...
    private readonly static SemaphoreSlim s_skinRequestGate = new(1, 1);
    private static DateTime s_nextSkinRequestUtc;

    private readonly static TimeSpan SkinRequestInterval = TimeSpan.FromSeconds(2);
    private readonly static TimeSpan RateLimitRetryDelay = TimeSpan.FromSeconds(10);
    ...
    public static bool HasCachedAvatar(MicrosoftAccount account)
    {
        return File.Exists(GetAvatarCachePath(account.Uuid));
    }
    ...
    public async static Task<bool> TryUpdateMicrosoftAvatarAsync(MicrosoftAccount account, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var skinUrlResult = await GetActiveSkinUrlWithRateLimitRetryAsync(account, cancellationToken);
    ...
    async private static Task<(string? skinUrl, Exception? exception)> GetActiveSkinUrlWithRateLimitRetryAsync(
        MicrosoftAccount account,
        CancellationToken cancellationToken)
    {
        for (var attempt = 1; attempt <= 3; attempt++)
        {
            var result = await RequestActiveSkinUrlAsync(account, cancellationToken);
            if (result.exception is not HttpRequestException
                {
            ...
            s_logger.Warn($"Microsoft avatar request for {account.Name} was rate limited. Retrying in {delay.TotalSeconds:0} seconds.");
            await Task.Delay(delay, cancellationToken);
        }

        throw new InvalidOperationException("The avatar retry loop did not complete.");
    }

    async private static Task<(string? skinUrl, Exception? exception)> RequestActiveSkinUrlAsync(
        MicrosoftAccount account,
        CancellationToken cancellationToken)
    {
        await s_skinRequestGate.WaitAsync(cancellationToken);
        try
        {
            var waitDuration = s_nextSkinRequestUtc - DateTime.UtcNow;
            if (waitDuration > TimeSpan.Zero)
            {
                await Task.Delay(waitDuration, cancellationToken);
            }

            var result = await MicrosoftOAuth.GetActiveSkinUrlAsync(account, cancellationToken);
            s_nextSkinRequestUtc = DateTime.UtcNow + (result.exception is HttpRequestException
                {
                    StatusCode: System.Net.HttpStatusCode.TooManyRequests
                }
                ? RateLimitRetryDelay
