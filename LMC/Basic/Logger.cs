...
namespace LMC.Basic
{
    public class Logger
    {
        ...
        public Logger(string module, string filePath = "not")
        {
            ...
            {
                filePath = Path.Combine(Current.LMCPATH, "logs", LogNum + ".log");
            }
            _logFile = filePath;
            ...
            try
            {
                while (!token.IsCancellationRequested || _logQueue.Count > 0)
                {
                    ...
                    if (_logQueue.TryTake(out logEntry, Timeout.Infinite, token))
                    {
                        ...
                        using (StreamWriter latestWriter = new StreamWriter(Path.Combine(Current.LMCPATH, "logs", "latest.log"), true))
                        {
        ...
        {
            var errorMsg=  $@"An exception occurred when {func}:
Caused by : {e.GetType().FullName} : {e.Message}
{e.StackTrace}
...
";
            while (true)
            {
                ...
                e = e.InnerException;
                errorMsg += $@"Caused by : {e.GetType().FullName} : {e.Message}
{e.StackTrace}
